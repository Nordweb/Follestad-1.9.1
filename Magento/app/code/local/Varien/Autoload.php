...
function __ewDependencyCheck() {
	if (@class_exists('Extendware_EWCore_Model_Autoload') === false) {
		die(sprintf('Cannot load file %s/app/code/local/Extendware/Model/Autoload.php. Please ensure all files have been uploaded. If your host is running software that deletes files after you upload them, then you will need to re-download the software and select one of the PHP versions with "ic" in the name.', BP));
	}
