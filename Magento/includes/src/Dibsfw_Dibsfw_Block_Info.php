...
class Dibsfw_Dibsfw_Block_Info extends Mage_Payment_Block_Info {
    ...
    public function getValueAsArray($value, $escapeHtml = false) {
        if(self::detectDibsCgiButtons($value) ||
           strpos($value, Mage::helper('dibsfw')->__('DIBSFW_LABEL_18')) !== FALSE) $escapeHtml = false;
    ...
    private static function detectDibsCgiButtons($sValue) {
        return (strpos($sValue, 'bof_dibsbuttons') !== FALSE ||
                strpos($sValue, 'eof_dibsbuttons') !== FALSE ||
                strpos($sValue, Mage::helper('dibsfw')->__('DIBSFW_LABEL_26')) !== FALSE);
    }
}
